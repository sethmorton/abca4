...
from .constants import (
    MISSENSE_SCORE_WEIGHT, ESM_SCORE_WEIGHT,
    AM_PATHOGENIC_THRESHOLD, AM_BENIGN_THRESHOLD,
    DEFAULT_AM_SCORE, DEFAULT_ESM_SCORE
)
...
class MissenseFeatureComputer:
    ...
    def add_esm_features(self, variants_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'esm_logit_diff' not in variants_with_esm.columns:
            ...
            if 'alphamissense_score' in variants_with_esm.columns:
                ...
                variants_with_esm['esm_logit_diff'] = DEFAULT_ESM_SCORE
    ...
    def compute_missense_features(self, variants_df: pd.DataFrame) -> pd.DataFrame:
        ...
        score = features_df['alphamissense_score'].fillna(DEFAULT_AM_SCORE)
        esm = features_df['esm_logit_diff'].fillna(DEFAULT_ESM_SCORE)

        features_df['alphamissense_pathogenic'] = (score > AM_PATHOGENIC_THRESHOLD).astype(int)
        features_df['alphamissense_benign'] = (score < AM_BENIGN_THRESHOLD).astype(int)
        features_df['missense_combined_score'] = MISSENSE_SCORE_WEIGHT * score + ESM_SCORE_WEIGHT * esm
