...
from .constants import (
    SPLICEAI_PATHOGENIC_THRESHOLD, SPLICEAI_HIGH_IMPACT_THRESHOLD,
    IMPACT_BINS, IMPACT_LABELS
)
...
class SpliceFeatureComputer:
    ...
    def compute_splice_features(self, variants_df: pd.DataFrame) -> pd.DataFrame:
        ...
        features_df['spliceai_pathogenic'] = (
            features_df['spliceai_max_score'] > SPLICEAI_PATHOGENIC_THRESHOLD
        ).astype(int)

        features_df['spliceai_high_impact'] = (
            features_df['spliceai_max_score'] > SPLICEAI_HIGH_IMPACT_THRESHOLD
        ).astype(int)
